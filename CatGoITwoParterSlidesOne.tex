...
\begin{frame}
	...
	\begin{example}
	The polynomail $U(x_1, x_2) = x_1^2 + x_2^2 \in \bb{C}[x_1, x_2]$ has no square root, but nonetheless
	\begin{equation*}
		\begin{pmatrix}
			0 & x_1 - ix_2\\
			x_1 + ix_2 & 0
...
\begin{frame}
	...
	The objects are polynomials which have isolated critical points (points where the rank of the Jacobian is not full).
	\begin{equation*}
...
\begin{frame}\frametitle{``Infinitary" compositions}
	...
	Define
	\begin{equation*}
		Y \circ X = (Y \otimes_{k[\underline{y}]} X, d_Y \otimes 1 + 1 \otimes d_X)
	\end{equation*}
...
\begin{frame}
	...
	A methodical process for recovering a matrix factorisation proper which is homotopy equivalent to the composite is the content of Murfet's paper \cite{cut_operation}. The relevant part of this process for today's purposes is the definition of the \textbf{cut} of two matrix factorisations.
	\begin{defn}
		...
		\begin{equation*}
			J_{V(\underline{y})} = \bb{C}[y_1, \ldots, y_m]/(\partial_{y_1}V(\underline{y}), \ldots, \partial_{y_m}V(\underline{y}))
			\end{equation*}
...
Recall that the Clifford Algebra $C_n$ is generated by elements $\mu_1, \ldots, \mu_n, \nu_1, \ldots, \nu_n$ subject to:
	\begin{equation*}
		...
		\end{equation*}
	where $[\xi, \zeta] = \xi\zeta + \zeta\xi$ for $\xi, \zeta \in \{ \mu_1, \ldots, \mu_n, \nu_1, \ldots, \nu_n \}$. Let $S_n$ denote $\bigwedge (\bb{C}\theta_1 \oplus \ldots \oplus \bb{C}\theta_n)$ where $\theta_1, \ldots, \theta_n$ are formal variables, and $n$ is the length of the sequence $\underline{y}$.
\end{frame}
	...
	\begin{lemma}
		...
		\begin{equation*}
			\begin{tikzcd}[ampersand replacement=\&]
				Y\otimes_{k[\underline{y}]} J_{V(\underline{y})} \otimes_{\bb{C}[\underline{y}]} X = Y \mid X\arrow[r,shift left] \& S_n \otimes_{\bb{C}} (Y \otimes X )\arrow[l,shift left]
			\end{tikzcd}
			\end{equation*}
		\end{lemma}
...
\begin{frame}
	...
	There is a $C_n$ action on $S_n$ and hence on $S_n \otimes_\bb{C} (Y \otimes X)$. This is induced by two canonical endomorphisms which exist on $S_n$. The \textbf{wedge} and \textbf{contraction} maps.
	\begin{align*}
...
\begin{frame}
	...
	Consider the endomorphism $e = \theta_1^\ast \ldots \theta_m^\ast \theta_m \ldots \theta_1: S_m \lto S_m$. This is the projection onto $k$ sitting inside $S_m$. Thus we obtain a pair of morphisms
	\begin{equation*}
		\begin{tikzcd}[ampersand replacement=\&]
			S_m \otimes_\bb{C} (Y \otimes X)\arrow[r,shift left,"{e}"] \& Y \otimes X\arrow[l, shift left, "{\iota}"]
			\end{tikzcd}
		\end{equation*}
	satisfying the properties that $e\iota = \operatorname{id}_{Y \otimes X}$ and $\iota e = e$. Carrying this through the homotopy
	\begin{equation*}
		Y \mid X \simeq S_m \otimes_\bb{C} (Y \otimes X)
		\end{equation*}
...
\begin{frame}
	...
	\[\begin{tikzcd}[ampersand replacement=\&]
		{Y \mid X} \& {S_m \otimes_\bb{C}(Y \otimes X)} \\
		{Y \circ X} \& {Y \otimes X}
...
\begin{frame}
	...
	Consider polynomails $\sum_{i = 1}^n x_i^2, \sum_{i = 1}^n y_i^2, \in \bb{C}[x_1, \ldots, x_n, y_1 \ldots, y_n]$.
\begin{lemma}
...
\begin{frame}
	\frametitle{The model, axiom and cut}
	Say $A$ has oriented atoms $(X_1, x_1), \ldots, (X_n, x_n)$.
	\begin{prooftree}
		\AxiomC{}
		\RightLabel{$\ax$}
		\UnaryInfC{$\vdash \neg A, A$}
		\end{prooftree}
	We define a matrix factorisation
	\begin{equation*}
		\operatorname{Ax}: (\varnothing, 0) \lto \llbracket \neg A \rrbracket \square \llbracket A \rrbracket
		\end{equation*}
	For each unoriented atom $X_i$ of $A$ there is a corresponding unoriented atom $X_i'$ of $\neg A$. We take the Koszul matrix factorisation corresponding to the sequence
	\begin{equation*}
		(X_1 - X_1', \ldots, X_n - X_n')
		\end{equation*}
	Our interpretation of $\cut$ is the same matrix factorisation $$\operatorname{Cut}: \llbracket A \rrbracket \square \llbracket \neg A \rrbracket \lto (\varnothing, 0)$$
	\end{frame}

\begin{frame}
	\frametitle{The model, tensor and par}
	Say $B$ has oriented atoms $(Y_1, y_1), \ldots, (Y_m, y_m)$.
	\begin{prooftree}
	...
	We define a matrix factorisation
	\begin{equation*}
	\operatorname{Tensor}: \llbracket A \rrbracket \square \llbracket B \rrbracket \lto \llbracket A \otimes B \rrbracket = \llbracket A \rrbracket \square \llbracket B \rrbracket
	\end{equation*}
Each unoriented atom $X_i$ of $A$ has a copy $X_i'$ in the unoriented atoms of $A \otimes B$, and similarly each unoriented atom $Y_j$ of $B$. We take the Koszul matrix factorisation corresponding to the sequence
\begin{equation*}
(X_1 - X_1', \ldots, X_n - X_n', Y_1 - Y_1', \ldots, Y_m - Y_m')
\end{equation*}
	This is a variant on the identity. Our interpretation of $(\operatorname{R} \parr)$ is exactly the same, we label it $\operatorname{Par}$.
	\end{frame}

\begin{frame}
	\frametitle{The model, exchange}
	Exchange is similar.
	\begin{prooftree}
	...
	We define a matrix factorisation
	\begin{equation*}
		\operatorname{Ex}: \llbracket A \rrbracket \square \llbracket B \rrbracket \lto \llbracket B \rrbracket \square \llbracket A \rrbracket
		\end{equation*}
	For each unoriented atom $Z$ in the combined sequence of those in $A$ and $B$, there is a corresponding unoriented atom $Z'$ in the combined sequence of those in $B$ and $A$. We take the Koszul matrix factorisation corresponding to the sequence
	\begin{equation*}
		(X_1 - X_1', \ldots, X_n - X_n', Y_1 - Y_1', \ldots, Y_m - Y_m')
		\end{equation*}
	\end{frame}
...
\begin{frame}
	\frametitle{Proofs as compositions of matrix factorisations}
	\begin{prooftree}
		\AxiomC{}
		\RightLabel{$\ax$}
		\UnaryInfC{$\vdash \neg A, A$}
		\RightLabel{$(\operatorname{R}\parr)$}
		\UnaryInfC{$\vdash \neg A \parr A$}
		\AxiomC{}
		\RightLabel{$\ax$}
		\UnaryInfC{$\vdash \neg A, A$}
		...
		\BinaryInfC{$\vdash \neg A, A$}
		\end{prooftree}
	This is interpreted as follows, the goal is to compute this composite using the cut (of matrix factorisations).
	\begin{equation*}
		\begin{tikzcd}[row sep = small]
			(\varnothing, 0) \square (\varnothing, 0)\arrow[d,"{\operatorname{Ax} \square \operatorname{Ax}}"]\\
			(\varnothing, 0) \square (\llbracket \neg A \rrbracket \square \llbracket A \rrbracket) \square (\llbracket \neg A \rrbracket \square \llbracket A \rrbracket)\arrow[d,"{\operatorname{Ax} \square \operatorname{id} \square \operatorname{Tensor} \square \operatorname{id}}"]\\
			(\llbracket \neg A \rrbracket \square \llbracket A \rrbracket) \square (\llbracket \neg A \rrbracket \square \llbracket A \otimes \neg A \rrbracket \square \llbracket A \rrbracket)\arrow[d,"{\operatorname{Par} \square \operatorname{Ex} \square \operatorname{id}}"]\\
			(\llbracket \neg A \parr A \rrbracket) \square (\llbracket A \otimes \neg A \rrbracket \square \llbracket \neg A \rrbracket \square \llbracket A \rrbracket)\arrow[d,"{\operatorname{Cut} \square \operatorname{id} \square \operatorname{id}}"]\\
			(\varnothing, 0) \square \llbracket \neg A \rrbracket \square \llbracket A \rrbracket
			\end{tikzcd}
		\end{equation*}
	\end{frame}
